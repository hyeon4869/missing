  ...
  with sqlite3.connect("info.db") as conn:
    ...
    while True:
      ...
      if face_distances < 0.35:
        ...
        #경로변경해야됨
        folder_name = matched_list[0]
        folder_name2 = matched_list[5]
        

        source = "라즈베리탐색이미지.png"
        
        destination = os.path.join("C:/temp/", folder_name)
        destination2 = os.path.join(destination + folder_name2)        
        ...
        with open(destination2 + "/test.txt", "w") as f:
